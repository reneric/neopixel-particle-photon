...
#define LED_TOTAL 200  //Change this to the number of LEDs in your strand.
...
void loop() {  //This is where the magic happens. This loop produces each frame of the visual.
  ...
  bump = (volume - last > avgBump * .4);

  //If a "bump" is triggered, average the time between bumps
  if (bump) {
    avgTime = (((millis() / 1000.0) - timeBump) + avgTime);
...
void Pulse() {
  ...
  fade(0.5);   //Listed below, this function simply dims the colors a little bit each pass of loop()
